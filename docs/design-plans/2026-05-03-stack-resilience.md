...
7. **Code organization.** Justfile recipes keep complex bash logic in scripts under `stacks/<stack>/scripts/` or `stacks/scripts/` rather than inline. The line count itself isn't the rule — the goal is readability. Simple multi-line recipes (per-stack iteration like `up-all`, straightforward `set -euo pipefail` bash bodies, multi-line user-facing echo blocks) are fine. Extract to a script when the body has hidden complexity, conditional branching that obscures intent, or shared logic that other recipes need to reuse. The new-device clean-setup path is statically validated end-to-end against the README + committed scripts; nothing relies on undocumented manual environment state.
...
- **stack-resilience.AC7.1 Success:** Justfile recipes keep complex bash logic out of the recipe body and in scripts under `stacks/scripts/` or `stacks/<stack>/scripts/`. The line count itself is not a hard limit — simple multi-line recipes (per-stack iteration like `up-all`, straightforward bash bodies for setup logic, multi-line user-facing echo blocks) are fine. Extract to a script when the body has hidden complexity, branching logic, or shared logic reused elsewhere.
- **stack-resilience.AC7.2 Success:** `stacks/scripts/` directory contains the stacks-shared scripts: `doctor.sh`, `backup.sh`, `restore.sh`, `backup-rotate.sh`, `bridged-ip-changed.sh`, `backup-install.sh`, `lib/check.sh`.
...
Code organization respects two rules: justfile recipes keep complex bash logic in scripts (line count is not a hard limit — simple multi-line recipes are fine; extract to a script when the body hides complexity or shares logic with other recipes); scripts colocate with their layer (stacks-shared in `stacks/scripts/`, per-stack in `stacks/<stack>/scripts/`). The launchd plist for nightly backups installs to `~/Library/LaunchAgents/` and is **not** part of stow — it's per-machine setup, distinct from config dotfiles, and the audience for stow is broader than the audience for the stacks side of the repo.
